...
public class ExportDbFit {

	private static final int EFFORT_THRESHOLD_DBFIT = -1;
	...
	public static void mineDbFit() throws Exception {
		Connection conn = Connection.getInstance();
		conn.connect("mongodb://localhost:27017", "visminer_dbfit");

		RepositoryMiner miner = new RepositoryMiner(props.getProperty("path.rep") + "dbfit", 
				props.getProperty("path.tmp"), "DbFit", "DbFit", SCMType.GIT);
		...
		miner.addPostMiningTask(
				new EffortByIssuesExporter(new EffortByIssueFilter(), EFFORT_THRESHOLD_DBFIT, props.getProperty("path.csv") + "dbfit.csv"));
		miner.mine();

		// Repository repository = miner.mine();
		//
		// HostingServiceMiner hsMiner = new
		// HostingServiceMiner(repository.getId(), "dbfit", "dbfit",
		...
		// props.getProperty("github.pass"), 516);
