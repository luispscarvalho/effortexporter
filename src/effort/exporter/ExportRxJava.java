...
public class ExportRxJava {

	private static final int EFFORT_THRESHOLD_RXJAVA = -1;
	...
	public static void mineRxJava() throws Exception {
		Connection conn = Connection.getInstance();
		conn.connect("mongodb://localhost:27017", "visminer_rxjava");

		RepositoryMiner miner = new RepositoryMiner(props.getProperty("path.rep") + "RxJava",
				props.getProperty("path.tmp"), "RxJava", "RxJava", SCMType.GIT);
		miner.setMiningListener(new ConsoleMiningListener());

		miner.addParser(new JavaParser());
		miner.addReference("2.x", ReferenceType.BRANCH);
		miner.setMiningListener(new ConsoleMiningListener());
		...
		miner.addPostMiningTask(new EffortByIssuesExporter(new EffortByIssueFilter(), EFFORT_THRESHOLD_RXJAVA,
				props.getProperty("path.csv") + "rxjava.csv"));
		miner.mine();

		// Repository repository = miner.mine();
		//
		// HostingServiceMiner hsMiner = new
		// HostingServiceMiner(repository.getId(), "ReactiveX", "RxJava",
		...
		// props.getProperty("github.pass"), 1, 1, 1001);
	}
